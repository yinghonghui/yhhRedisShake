...
import (
	...
	"redis-shake/redis-shake/base"
	"redis-shake/redis-shake/common"
	"redis-shake/redis-shake/heartbeat"
	"redis-shake/redis-shake/metric"

	"redis-shake/redis-shake/checkpoint"
...
func (ds *DbSyncer) Del() {
	...
	if isFullSync {
		...
		log.Infof("DbSyncer[%d] rdb file size = %d", ds.id, nsize)
		base.Status = "full"
		ds.delRDBFile(reader, ds.target, conf.Options.TargetAuthType, ds.targetPassword, nsize, conf.Options.TargetTLSEnable)
		ds.startDbId = 0
	} else {
		log.Infof("DbSyncer[%d] run incr-sync directly with db_id[%v]", ds.id, dbid)
		ds.startDbId = dbid
